...
export async function getArticles(options?: { search?: string; status?: string }) {
  'use cache';
...
export async function getArticle(id: string) {
  'use cache';
  cacheTag(`article-id-${id}`);
  cacheLife('minutes');
  return db.select().from(article).where(eq(article.id, id)).then(rows => rows[0]);
}
...
export async function getArticleBySlug(slug: string) {
  ...
  cacheLife('minutes');
  return db.select().from(article).where(eq(article.slug, slug)).then(rows => rows[0]);
}
